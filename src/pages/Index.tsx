...
import { getTrendingMovies, getTrendingShows, getUpcomingMovies, getNowPlayingMovies, getPosterUrl, getBackdropUrl, GENRE_MAP } from "@/lib/tmdb";
      ...
      <div className="relative max-w-7xl mx-auto px-4 h-full flex items-end pb-12">
        <div className="max-w-xl animate-fade-in">
          ...
          <div className="flex items-center gap-3">
            <Link
              to={`/title/${hero.id}?type=${hero.title ? 'movie' : 'show'}`}
              className="bg-primary text-primary-foreground px-5 py-2.5 rounded-lg text-sm font-semibold hover:bg-hub-red-hover transition-colors"
            >
              View Details
            </Link>
          ...
          <div className="flex gap-2 mt-6">
            {items.slice(0, 5).map((_, i) => (
              <button
                key={i}
                onClick={() => setActive(i)}
                className={`h-1 rounded-full transition-all ${i === active ? 'w-8 bg-primary' : 'w-4 bg-muted-foreground/30'}`}
              />
            ))}
          </div>
        </div>
    ...
    <div className="bg-card rounded-xl border border-hub-border p-4">
      ...
      <div className="space-y-3">
        {items.slice(0, 10).map((item, i) => (
          <Link
            key={item.id || i}
            to={`/title/${item.id}?type=${item.title ? 'movie' : 'show'}`}
            className="flex items-center gap-3 group"
          >
            <span className={`text-lg font-black w-6 text-center ${i < 3 ? 'text-primary' : 'text-muted-foreground'}`}>
              {i + 1}
            </span>
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-muted-foreground">
                {item.genre_ids ? GENRE_MAP[item.genre_ids[0]] : ''} · {(item.release_date || item.first_air_date || '').slice(0, 4)}
              </p>
            </div>
          </Link>
        ))}
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-10">
            ...
            {/* Trending Shows */}
            <section>
              <SectionHeader title="Trending Shows" href="/shows" />
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {loading
                  ? Array.from({ length: 6 }).map((_, i) => <ContentCardSkeleton key={i} />)
                  : trendingShows.slice(0, 6).map((item: any) => (
                      <ContentCard
                        key={item.id}
                        id={item.id}
                        title={item.name}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  {topAnime.slice(0, 6).map((item: any) => (
                    <ContentCard
                      ...
                      score={item.score}
                      href={`/title/${item.mal_id}?type=anime`}
                    />
                  ))}
                </div>
              </section>
